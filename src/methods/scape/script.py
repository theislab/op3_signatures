...
from src.methods.scape.cli import parse_args

par = parse_args()


def write_predictions(df_submission_data, par, de_train, id_map):
...
model_dir = par["output_model"] or tempfile.mkdtemp(dir="/tmp")

if not par["output_model"]:
    import atexit
    atexit.register(lambda: shutil.rmtree(model_dir, ignore_errors=True))
...
if len(top_drugs) == 0:
    ...
    write_predictions(df_submission_data, par, de_train, id_map)
    sys.exit(0)
