...
import pandas as pd
import numpy as np
from src.methods.lgc_ensemble_helpers.helper_functions import combine_features, train_validate
...
def train(par, paths):
    ...
    X_vec = combine_features([mean_cell_type, std_cell_type, mean_sm_name, std_sm_name],\
                [train_chem_feat, train_chem_feat_mean], de_train_df, one_hot_train)
    ...
    X_vec_heavy = combine_features([quantiles_df,mean_cell_type,mean_sm_name],\
                    [train_chem_feat,train_chem_feat_mean], de_train_df, one_hot_train, quantiles_df)
    ## Start training
    ...
    print(f"de_train:{de_train_df.shape}")
    print(f"Y:{y.shape}")
