...
def model_1(lr, emb_out, n_dim, use_fp_dense):
    ...
    fp_input = Input(shape=(2000,), name="fp_input")
...
def predict(test_df, models, params, weights, le, new_names, train_fps, original_y, reps, use_fp_dense):
    ...
    fp_test = smiles_to_fingerprints(
        test_df["SMILES"].astype(str).fillna("") if "SMILES" in test_df.columns
        else [""] * len(test_df)
    ).astype(np.float32)
...
def run_notebook_266(train_df, test_df, pseudolabel, gene_names, reps, use_fp_dense=True):
    ...
    train_fps = smiles_to_fingerprints(
        train_df["SMILES"].astype(str).fillna("")
    ).astype(np.float32)
