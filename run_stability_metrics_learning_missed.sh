...
# ============================================================================
# Run metrics for all nn_retraining_with_pseudolabels_mol_emb_learning_missed_subsample*
...
RESULTS_DIR="./data_lpm_style_learning_missed/benchmark/results/methods"
METHOD_PATTERN="nn_retraining_with_pseudolabels_mol_emb_learning_missed_subsample"
...
for method_dir in "${RESULTS_DIR}/${METHOD_PATTERN}"*/; do
    ...
    for stability_dir in "${method_dir}"stability*/; do
        ...
        ./run_pipelines/run_metrics_stability_learning_missed.sh \
            -m "${method_name}" \
            -b "${subfolder}"
    done
done
